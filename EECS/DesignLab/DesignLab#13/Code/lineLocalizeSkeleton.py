...
def makeRobotNavModel(ideal, xMin, xMax, numStates, numObservations):
    ...
    def observationModel(ix):
        ...
        NoiseDist = dist.squareDist(0,numObservations)
        peakDist = dist.triangleDist(ideal[ix],round(numObservations/2.50))
        MaxDist = dist.DeltaDist(numObservations)
        dist1 = dist.MixtureDist(peakDist,NoiseDist,float(8000)/8800)
        dist2 = dist.MixtureDist(dist1, MaxDist,float(8800)/10000) # these data comes from the plot
        return dist2
